...
class GamestaggingController extends \Phalcon\Mvc\Controller {
    ...
    public function getbyid($id = null) {
        ...
        if (empty($id)):
            ...
            $collection = GamesTagging::findFirstByid($id);
            if ($collection):
    ...
    /**
     * This function using to create GamesTagging information
     */
    public function create() {
        ...
        $validation->add('games_database_id', new PresenceOf(['message' => 'games_database_id is required']));
        ...
        if (count($messages)):
            ...
            $collection = new GamesTagging();
            $collection->id = $input_data->id;
            $collection->indicators_id = $input_data->indicators_id;
            $collection->games_database_id = $input_data->games_database_id;
            if ($collection->save()):
    ...
    /**
     * This function using to GamesTagging information edit
     */
    public function update($id = null) {
        ...
        if (empty($id)):
            ...
            $validation->add('games_database_id', new PresenceOf(['message' => 'games_database_idis required']));
            $messages = $validation->validate($input_data);
            if (count($messages)):
                foreach ($messages as $message):
                    $result[] = $message->getMessage();
